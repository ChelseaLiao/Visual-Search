...
namespace VRception
{
    public class SelectRayCaster_Training : MonoBehaviour
    {
        [SerializeField]
        float maxRaycasterLength = 12.0f;
        [SerializeField] TrainingEnvironment env;
            ...
            if (hit.collider != null)
            {
                ...
                if (clickAction.GetStateUp(targetSource))
                {
                    ...
                    if (hit.transform.CompareTag("Target"))
                    {
                        ...
                        Debug.Log("log click time to target at" + clickTime);
                        IsTarget = true;
                        accuracy++;
                        IsInteracted = true;
                    }
                    ...
                    {
                        clickTime = System.DateTimeOffset.Now.ToUnixTimeMilliseconds();
                        Debug.Log("log click time to wrong item at" + clickTime);
                        IsTarget = false;
                        IsInteracted = true;
                    }
